    ...
    <div className="min-h-screen flex flex-col relative z-10 text-[var(--text-primary)] transition-colors duration-400 selection:bg-sky-500 selection:text-white">
      ...
      <main className="flex-grow pt-28 max-w-7xl mx-auto w-full px-4 sm:px-6">
        ...
        {activeTab === 'calendar' && (
          <div className="animate-fade-in py-6">
            <div className="mb-6">
              <h1 className="text-2xl font-bold font-heading tracking-tight text-[var(--text-primary)]">Deadlines & Calendar</h1>
              <p className="text-xs text-[var(--text-secondary)] mt-1">Track target submission dates and register new assignment dates.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              {/* Calendar Grid */}
              <div className="lg:col-span-8 p-5 tsec-glass-card">
                <div className="flex justify-between items-center mb-5 border-b border-[var(--border-color)] pb-3">
                  <h3 className="font-extrabold text-sm text-[var(--text-primary)] font-heading uppercase tracking-wider">June 2026</h3>
                  <span className="text-[10px] font-mono font-bold text-[var(--text-muted)]">Base Date: June 26, 2026</span>
                </div>

                <div className="grid grid-cols-7 gap-1.5 text-center font-mono">
                  {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map(h => (
                    <div key={h} className="text-[10px] font-bold text-[var(--text-muted)] py-1.5">{h}</div>
                  ))}
                  ...
                  {calendarCells.map((cell, idx) => (
                    <div 
                      key={idx}
                      onClick={() => {
                        if (cell.isCurrent) setSelectedCalendarDate(cell);
                      }}
                      className={`aspect-square flex flex-col justify-between p-1.5 rounded-xl border text-xs cursor-pointer select-none transition-all ${
                        cell.isCurrent 
                        ...
                        cell.isToday 
                          ? 'border-sky-500 text-sky-500 font-bold bg-sky-500/5 shadow-inner' 
                          : ''
                      } ${
                        ...
                        selectedCalendarDate?.dateStr === cell.dateStr
                          ? 'ring-2 ring-sky-500/50'
                          : ''
                      }`}
                    >
                      <div className="text-left text-[9px] font-mono font-bold">{cell.day}</div>
                      {cell.deadlines?.length > 0 && (
                        <div className="flex justify-center mb-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse pulse-glow"></span>
                        </div>
                      )}
                    </div>
                  ))}
              ...
              <div className="lg:col-span-4 flex flex-col gap-4">
                ...
                <div className="p-5 tsec-glass-card">
                  <h3 className="font-bold text-xs mb-3 text-[var(--text-primary)] font-heading">Add Target Deadline</h3>
                  <form onSubmit={handleAddDeadline} className="flex flex-col gap-3">
                    <input 
                      required 
                      type="text" 
                      placeholder="Assignment / Lab Name" 
                      value={deadlineTitle} 
                      onChange={(e) => setDeadlineTitle(e.target.value)} 
                      className="p-2.5 text-xs rounded-lg border border-[var(--border-color)] bg-[var(--bg-primary)] outline-none focus:border-sky-500 w-full"
                    />
                    <input 
                      required 
                      type="text" 
                      ...
                      value={deadlineSub} 
                      onChange={(e) => setDeadlineSub(e.target.value)} 
                      className="p-2.5 text-xs rounded-lg border border-[var(--border-color)] bg-[var(--bg-primary)] outline-none focus:border-sky-500 w-full"
                    />
                    <input 
                      required 
                    ...
                    <button 
                      type="submit" 
                      className="w-full py-2.5 bg-sky-500 hover:bg-sky-600 text-white font-bold text-xs rounded-lg transition-all cursor-pointer"
                    >
                      Pin Deadline
                    </button>
                  </form>
                ...
                {/* List items */}
                <div className="p-5 tsec-glass-card">
                  <h3 className="font-bold text-xs mb-3 text-[var(--text-primary)] font-heading">Upcoming Targets</h3>
                  <div className="flex flex-col gap-2.5">
                    {selectedCalendarDate && selectedCalendarDate.deadlines?.length > 0 ? (
                      <div className="mb-2 p-3 bg-rose-500/5 rounded-xl border border-rose-500/25">
                        <div className="text-[10px] font-bold text-rose-500 mb-1">Due on {selectedCalendarDate.dateStr}:</div>
                        {selectedCalendarDate.deadlines.map(dl => (
                          <div key={dl.id} className="text-xs font-bold text-[var(--text-primary)] mb-1 last:mb-0">
                            • {dl.title}
                          </div>
                        ))}
                      </div>
                    ...
                    {deadlines.length === 0 ? (
                      <p className="text-xs text-[var(--text-muted)] font-medium">No deadlines currently tracked.</p>
                    ) : (
                      deadlines.slice(0, 4).map(d => (
                        <div key={d.id} className="p-3 rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)]/50 flex items-center justify-between">
                          <div>
                            <div className="text-xs font-bold text-[var(--text-primary)] leading-tight">{d.title}</div>
                            <div className="text-[9px] text-[var(--text-secondary)] mt-1 font-semibold">{d.subject.toUpperCase()}</div>
                          </div>
